...
RSpec.describe Yabeda::Puma::Plugin::Statistics::Parser do
  describe '#parse' do
          ...
          'worker_status' => [
            { 'pid' => 13, 'index' => 0, 'phase' => 0, 'booted' => true, 'last_checkin' => '2019-03-31T13:04:28Z',
              'last_status' => {
                ...
                'requests_count' => 1, 'requests_wait_time' => 5
              } },
            { 'pid' => 17, 'index' => 1, 'phase' => 0, 'booted' => true, 'last_checkin' => '2019-03-31T13:04:28Z',
              'last_status' => {
                'backlog' => 0, 'running' => 5, 'pool_capacity' => 5, 'max_threads' => 5,
                'requests_count' => 2, 'requests_wait_time' => 6
              } }
          ]
        }
      end
      ...
      it do
        expect(statistics).to match_array(
          [
            ...
            { name: 'requests_count', labels: { index: 0 }, value: 1 },
            { name: 'requests_wait_time', labels: { index: 0 }, value: 5 },
            ...
            { name: 'requests_count', labels: { index: 1 }, value: 2 },
            { name: 'requests_wait_time', labels: { index: 1 }, value: 6 }
          ]
        )
      end
    end
    ...
    context 'when puma is unclustered' do
      let(:clustered) { false }
      let(:data) do
        { 'backlog' => 0, 'running' => 5, 'pool_capacity' => 4, 'max_threads' => 5, 'requests_count' => 1,
          'requests_wait_time' => 5 }
      end
